...
/// This struct demonstrates SwiftyLua's thread safety and compatibility with Swift concurrency
@Suite("Swift Concurrency with SwiftyLua")
struct SwiftConcurrencyTests {

  @Test("Create multiple VMs sequentially to demonstrate thread safety")
  func createMultipleVMsSequentially() throws {
    var vms: [LuaVM] = []
    ...
    for (index, vm) in vms.enumerated() {
      ...
      if case VirtualMachine.EvalResults.values(let returnValues) = result {
        ...
        #expect(vmId == Int64(index))
      }
    }
  }

  @Test("Demonstrate VM isolation and thread safety")
  func vmIsolationAndThreadSafety() throws {
    ...
    {
      #expect(returnValues1[0] as? String == "VM1")
      #expect(returnValues2[0] as? String == "VM2")
      #expect(returnValues3[0] as? String == "VM3")
    } else {
      Issue.record("Expected string return values")
    }
  }

  @Test("Demonstrate async function registration")
  func asyncFunctionRegistration() throws {
    let vm = LuaVM(openLibs: true)
    ...
    if case VirtualMachine.EvalResults.values(let returnValues) = result {
      #expect(returnValues[0] as? String == "Delayed execution completed")
    } else {
      Issue.record("Expected a return value")
    }
  }

  @Test("Use Mutex-based VM registry safely")
  func mutexBasedVMRegistry() {
    let vm1 = LuaVM(openLibs: true)
    let vm2 = LuaVM(openLibs: true)

    // Both VMs should be properly registered without data races
    #expect(vm1.vm.state != nil)
    #expect(vm2.vm.state != nil)
    #expect(vm1.vm.state != vm2.vm.state)
  }

  @Test("Create isolated VMs concurrently using TaskGroup")
  func createIsolatedVMsConcurrently() async {
