//
//  LuaVMRegisterCustomTypeTests.swift
//  SwiftyLua
//
    ...
    return CustomTypeDescriptor(
      ...
      methods: [
        MethodDescriptor("getName") {
          (instance: CustomTypeImplementation, args: Arguments) -> SwiftReturnValue in
          let car = instance as! Car
          return .value(car.name)
        },
        MethodDescriptor("setName", parameters: [String.arg]) {
          (instance: CustomTypeImplementation, args: Arguments) -> SwiftReturnValue in
          let car = instance as! Car
...
@Suite("Register Custom Type")
struct LuaVMRegisterCustomTypeTests {

  @Test("Create and register custom type, then execute script")
  func registerAndExecuteScript() throws {
    ...
    _ = try vm
      .execute(
        url: Bundle.module.url(
          forResource: "register_custom_type", withExtension: "lua", subdirectory: "LuaScripts")!)
  }

  @Test("Retrieve Car object after execution")
  func retrieveCarObject() throws {
    let vm = LuaVM(openLibs: true)
    vm.registerCustomType(type: Car.self)
    ...
    if case VirtualMachine.EvalResults.values(let returnValue) = try vm.execute(string: "return myCar;") {
      ...
      #expect(car.name == "Volvo")
    } else {
      Issue.record("Unexpected result")
    }
  }

  @Test("Retrieve Car name using method call")
  func retrieveCarNameViaMethod() throws {
    let vm = LuaVM(openLibs: true)
    vm.registerCustomType(type: Car.self)
    ...
    if case VirtualMachine.EvalResults.values(let returnValue) = try vm.execute(
      string: "return myCar:getName()")
    {
