...
private class UsageCar: CustomTypeImplementation {
...
struct UsageTests {
  ...
  @Suite("Instantiating a Lua virtual machine")
  struct InstantiatingVM {

    @Test("Instantiate a Lua VM without loading the Lua standard library")
    func instantiateWithoutLibs() {
      let _ = LuaVM(openLibs: false)
    }

    @Test("Instantiate a Lua VM with loading the Lua standard library")
    func instantiateWithLibs() {
      let _ = LuaVM()
    }
  }

  @Suite("Executing Lua code")
  struct ExecutingCode {

    @Test("Execute code from a string")
    func executeFromString() throws {
      let lua = LuaVM()
    ...
    @Test("Execute code from a file")
    func executeFromFile() throws {
      let lua = LuaVM()
      let result =
        try lua.execute(
  ...
  @Suite("Setting global variables and structs")
  struct GlobalVariables {

    @Test("Register a global variable")
    func registerGlobalVariable() throws {
      let lua = LuaVM()
    ...
    @Test("Register a global struct")
    func registerGlobalStruct() throws {
      let lua = LuaVM()
      ...
      if case VirtualMachine.EvalResults.values(let returnValue) = result {
        NSLog(
          "(x, y) = (\((returnValue[0] as! Number).toDouble()), \((returnValue[1] as! Number).toDouble())) "
        )
      }
    }
  }

  @Suite("Registering a native Swift function")
  struct RegisterFunction {

    @Test("Register a native Swift function and call it")
    func registerAndCallFunction() throws {
      let lua = LuaVM()
  ...
  @Suite("Registering a native Swift type (class or struct)")
  struct RegisterCustomType {

    @Test("Register a native Swift type and instantiate it")
    func registerAndInstantiateType() throws {
      let lua = LuaVM()

      lua.registerCustomType(type: UsageCar.self)
      let result = try lua
        .execute(string: """
          myCar = UsageCar.new();
      ...
      if case VirtualMachine.EvalResults.values(let returnValue) = result {
        let car: UsageCar = (returnValue[0] as! Userdata).toCustomType()
